...
public static class DataProtectionKeyOptions
{
        ...
        IHostEnvironment environment)
    {
        ...
        var keysPath = configuration["DATA_PROTECTION_KEYS_PATH"];
        if (string.IsNullOrWhiteSpace(keysPath))
        {
            throw new InvalidOperationException(
                "DATA_PROTECTION_KEYS_PATH is not configured. Persistent Data Protection keys are required outside Development.");
        }

        var directory = EnsureWritableDirectory(keysPath, environment.ContentRootPath, "DATA_PROTECTION_KEYS_PATH");
